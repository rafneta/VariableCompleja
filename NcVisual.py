...
def Granumim(uno,dos,inverso, malla,               actilim, xmin, xmax, ymin,              ymax, real1=1.5,imag1=-2.5,real11=1.5,imag11=-2.5, guarda=False):
    ...
    if inverso==True:
        ax.plot([0, re(zi)],[0,im(zi)],color=cp[1],lw=3,                label=r"Inverso multiplicativo: "+r"$z^{-1}=(%s$ "%latex(re(zi))+"$,%s)$" %latex(im(zi)) )
...
def Granumsuma(suma,malla,
    ...
    if suma==True:
        ax.plot([re(z), re(zsuma)],[im(z),im(zsuma)],'--',color=cp[1],lw=1)
        ax.plot([re(z2), re(zsuma)],[im(z2),im(zsuma)],'--',color=cp[0],lw=1)
        ax.plot([0, re(zsuma)],[0,im(zsuma)],color=cp[2],lw=3,label="Suma: $z_1+z_2=(%s$" % latex(re(zsuma))+                                                        "$,%s)$" % latex(im(zsuma)))
        ax.plot(re(zsuma),im(zsuma),color=cp[2], marker=marcador,markersize=tam_marcador)
        #plt.legend(loc = "best")
        plt.legend(bbox_to_anchor=(1.3, 0.1,ancho_cuadro, 1), loc=3,           ncol=1, mode="expand", borderaxespad=0., fontsize=13)
...
def ncsuma(real1=1.5,imaginario1=-2.5,real2=-1,imaginario2=-0.5):
    style = {'description_width': 'initial'}
    global guardanota
    guardanota=widgets.Label(
        value=""
    ...
    #display(titulo)
    display(interactive_output(Granumsuma,{'real1':r1,         'imag1':ima1, 'real2':r2,'imag2':ima2, 'suma':Suma,         'malla':Malla,'actilim':Actilim, 'xmin':Xmin, 'xmax':Xmax, 'ymin':Ymin,                                           'ymax':Ymax, }))
    ...
    def on_guardar_clicked(b):
        guardanota.value="IMAGEN GUARDADA"
        Granumsuma(suma=Suma.value, malla=Malla.value,               actilim=Actilim.value, xmin=Xmin.value, xmax=Xmax.value, ymin=Ymin.value,              ymax=Ymax.value, real1=r1.value,imag1=ima1.value,real2=r2.value,imag2=ima2.value,
               guarda=True)
...
def Granumresta(resta,malla,
    ...
    if resta==True:
        ax.plot([0, re(-1*z2)],[0,im(-1*z2)],color=cp[2],lw=3,label=r"$-z_2=(%s$"%latex(re(-1*z2))                +"$,%s)$" % latex(im(-1*z2)))
        ax.plot(re(-1*z2),im(-1*z2),color=cp[2], marker=marcador,markersize=tam_marcador)
        
        ax.plot([re(z), re(zresta)],[im(z),im(zresta)],'--',color=cp[3],lw=1)
        ax.plot([re(-1*z2), re(zresta)],[im(-1*z2),im(zresta)],'--',color=cp[0],lw=1)
        
        ax.plot([0, re(zresta)],[0,im(zresta)],color=cp[3],lw=3,label=r"Resta: $z_1-z_2=(%s$"% latex(re(zresta))                +"$,%s)$" % latex(im(zresta)))
        ax.plot(re(zresta),im(zresta), color=cp[3], marker=marcador,markersize=tam_marcador)
        #plt.legend(loc = "best")
        plt.legend(bbox_to_anchor=(1.3, 0.1, ancho_cuadro, 1), loc=3,           ncol=1, mode="expand", borderaxespad=0., fontsize=13)
...
def ncresta(real1=1.5,imaginario1=-2.5,real2=-1,imaginario2=-0.5):
    style = {'description_width': 'initial'}
    global guardanota
    guardanota=widgets.Label(
        value=""
    ...
    #display(titulo)
    display(interactive_output(Granumresta,{'real1':r1,         'imag1':ima1, 'real2':r2,'imag2':ima2, 'resta':Resta,         'malla':Malla,'actilim':Actilim, 'xmin':Xmin, 'xmax':Xmax, 'ymin':Ymin,                                           'ymax':Ymax, }))
    ...
    def on_guardar_clicked(b):
        guardanota.value="IMAGEN GUARDADA"
        Granumresta(resta=Resta.value, malla=Malla.value,               actilim=Actilim.value, xmin=Xmin.value, xmax=Xmax.value, ymin=Ymin.value,              ymax=Ymax.value, real1=r1.value,imag1=ima1.value,real2=r2.value,imag2=ima2.value,
               guarda=True)
...
def Granumdivision(division,malla,
               actilim, xmin, xmax, ymin,\
              ymax, real1=1.5,imag1=-2.5,real2=-1,imag2=-0.5,guarda=False):
    ...
    if z2==0 and division==True:
        division=False
        ax.axis([-0.3,1,-0.3,1])
        ax.text(0.05, 0.9, r"No es posible obtener la"  ,                 horizontalalignment='left', rotation=0, fontsize=13, color='red')
        ax.text(0.05, 0.5, r"division si $z_2=0$"  ,                 horizontalalignment='left', rotation=0, fontsize=13, color='red')
    ...
    if division==True:
        ax.plot([0, re(zdiv)],[0,im(zdiv)],color=cp[2], lw=3,                label=r"Division: $\frac{z_1}{z_2}== (%s$" % latex(re(zdiv))+                                                        "$,%s)$" % latex(im(zdiv)))
        ax.plot(re(zdiv),im(zdiv), color=cp[2], marker=marcador,markersize=tam_marcador)
        #plt.legend(loc = "best")
        plt.legend(bbox_to_anchor=(1.3, 0.1, ancho_cuadro, 1), loc=3,           ncol=1, mode="expand", borderaxespad=0., fontsize=13)
        #(x0, y0, width, height)
...
def ncdivision(real1=1.5,imaginario1=-2.5,real2=-1,imaginario2=-0.5):
    style = {'description_width': 'initial'}
    global guardanota
    guardanota=widgets.Label(
        value=""
    ...
    #display(titulo)
    display(interactive_output(Granumdivision,{'real1':r1,         'imag1':ima1, 'real2':r2,'imag2':ima2, 'division':Division,         'malla':Malla,'actilim':Actilim, 'xmin':Xmin, 'xmax':Xmax, 'ymin':Ymin,                                           'ymax':Ymax, }))
    ...
    def on_guardar_clicked(b):
        guardanota.value="IMAGEN GUARDADA"
        Granumdivision(division=Division.value, malla=Malla.value,               actilim=Actilim.value, xmin=Xmin.value, xmax=Xmax.value, ymin=Ymin.value,              ymax=Ymax.value, real1=r1.value,imag1=ima1.value,real2=r2.value,imag2=ima2.value,
               guarda=True)
